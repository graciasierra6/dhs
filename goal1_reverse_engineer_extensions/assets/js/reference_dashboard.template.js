(() => {
  ...
  const nationalPrevalenceRows = {{NATIONAL_PREVALENCE_ROWS}};
  ...
  const referenceContrastColors = { "#F7FCFD":"#7EA7C7", "#E0ECF4":"#719ABD", "#BFD3E6":"#668FB2", "#9EBCDA":"#5B83A8" };
  const referenceColorForValue = (value, min, max, reverse = false) => {
    const color = colorForValue(value, min, max, reverse).toUpperCase();
    return referenceContrastColors[color] || color;
  };
  ...
  function updateIndicator(id, section) {
    ...
    const medianLabel = section.querySelector('[data-role="indicator-median-label"]');
    medianLabel.textContent = "Median across states";
    medianLabel.title = "Unweighted median of the displayed state values; this is not the national prevalence.";
    ...
    const distributionPanel = section.querySelector(".bar-panel");
    distributionPanel.classList.add("national-reference-panel");
    distributionPanel.querySelector(".eyebrow").textContent = "State ranking: high to low";
    section.querySelector('[data-role="bar-title"]').textContent = definition.label;
    section.querySelector('[data-role="bar-count"]').textContent = rows.length + " states";
    ...
    const nationalReference = nationalPrevalenceRows.find((row) => row.country === country && row.indicator === definition.id);
    if (!nationalReference) throw new Error("Missing national prevalence for " + country + " / " + definition.id);
    const countryLabel = country === "drc" ? "DRC" : country.charAt(0).toUpperCase() + country.slice(1);
    let referenceSummary = distributionPanel.querySelector(".national-reference-summary");
    if (!referenceSummary) {
      referenceSummary = document.createElement("div");
      referenceSummary.className = "national-reference-summary";
      distributionPanel.querySelector(".bar-panel-head").insertAdjacentElement("afterend", referenceSummary);
    }
    referenceSummary.innerHTML = '<strong data-role="national-prevalence-label"></strong><span>Vertical line denotes national prevalence.</span>';
    referenceSummary.querySelector('[data-role="national-prevalence-label"]').textContent = countryLabel + " national prevalence: " + fmt(nationalReference.nationalPrevalence) + "%";

    const domainValues = values.concat([Number(nationalReference.nationalPrevalence)]);
    const observedMin = Math.min(...domainValues), observedMax = Math.max(...domainValues);
    const padding = Math.max(1, (observedMax - observedMin) * 0.07);
    const domainMin = Math.max(0, observedMin - padding), domainMax = Math.min(100, observedMax + padding);
    const position = (value) => domainMax === domainMin ? 50 : Math.max(0, Math.min(100, 100 * (value - domainMin) / (domainMax - domainMin)));
    const referencePosition = position(Number(nationalReference.nationalPrevalence));
    barsContainer.style.setProperty("--national-position", referencePosition + "%");

    const columnHeader = document.createElement("div");
    columnHeader.className = "reference-column-header";
    columnHeader.innerHTML = "<span>State</span><i aria-hidden=\"true\"></i><strong>Prevalence (%)</strong>";
    barsContainer.appendChild(columnHeader);
    const referenceRows = document.createElement("div");
    referenceRows.className = "reference-rows";
    barsContainer.appendChild(referenceRows);
    rows.forEach((row) => {
      const barRow = document.createElement("div");
      barRow.className = "bar-row reference-row";
      const nameSpan = document.createElement("span");
      nameSpan.textContent = row.adminName;
      nameSpan.title = row.adminName;
      barRow.appendChild(nameSpan);
      const trackDiv = document.createElement("div");
      trackDiv.className = "reference-track";
      const valuePosition = position(Number(row.observedLatestEstimate));
      const connector = document.createElement("i");
      connector.className = "reference-connector";
      connector.style.left = Math.min(valuePosition, referencePosition) + "%";
      connector.style.width = Math.abs(valuePosition - referencePosition) + "%";
      const referenceMarkColor = referenceColorForValue(row.observedLatestEstimate, min, max, definition.direction === "beneficial");
      connector.style.backgroundColor = referenceMarkColor;
      trackDiv.appendChild(connector);
      const point = document.createElement("b");
      point.className = "reference-point";
      point.style.left = valuePosition + "%";
      point.style.backgroundColor = referenceMarkColor;
      trackDiv.appendChild(point);
      barRow.appendChild(trackDiv);
      const valueStrong = document.createElement("strong");
      valueStrong.textContent = fmt(row.observedLatestEstimate) + "%";
      barRow.appendChild(valueStrong);
      barRow.setAttribute("aria-label", row.adminName + ", " + fmt(row.observedLatestEstimate) + " percent; " + fmt(Math.abs(row.observedLatestEstimate - nationalReference.nationalPrevalence)) + " percentage points " + (row.observedLatestEstimate >= nationalReference.nationalPrevalence ? "above" : "below") + " national prevalence");
      referenceRows.appendChild(barRow);
    });
  }
