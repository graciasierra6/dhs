...
reference_validate_tab_panel_structure <- function(html, include_methods = FALSE) {
  ...
  required_tabs <- c("overview", "bivariate", if (isTRUE(include_methods)) "methods")
  expected <- as.vector(outer(c("drc", "ethiopia", "nigeria"), required_tabs, paste, sep = "|"))
  if (!identical(sort(panels), sort(expected))) {
    stop(
      "Dashboard tab panels do not match the required country-tab combinations. Found: ",
...
reference_validate_count_map <- function(report, html) {
  ...
  if (grepl('class="map-source"', section, fixed = TRUE)) {
    stop(report$country_name, " count map includes a source caption that should be hidden.", call. = FALSE)
  }
  if (reference_fixed_count('<use class="map-region', section) != report$rank_max) {
...
reference_validate_output_payloads <- function(html, reports, project_root = ".") {
  ...
  output_mortality <- reference_extract_output_json(html, "mortalityRows", "nationalPrevalenceRows")
  output_national_prevalence <- reference_extract_output_json(html, "nationalPrevalenceRows", "purpleScale")
  reference_compare_frames(
    generated$indicator_rows,
    output_indicators,
    keys = c("country", "indicator", "adminName"),
  ...
  expected_national_prevalence <- read_national_prevalence_reference(
    file.path(project_root, "data", "national_prevalence_reference.csv"),
    generated$indicator_rows
  )
  reference_compare_frames(
    expected_national_prevalence,
    output_national_prevalence,
    keys = c("country", "indicator"),
    text_fields = c(
      "sourceType", "reportTitle", "reportId", "tableId", "tableRow",
      "indicatorDefinition", "reportUrl", "validationNote"
    ),
    numeric_fields = c("latestYear", "nationalPrevalence")
  )
  invisible(TRUE)
}

reference_validate_html <- function(html, reports, output = FALSE) {
  reference_validate_tab_panel_structure(html, include_methods = output)
  required_sections <- c(
  ...
  if (output) required_sections <- c(required_sections, "drc-methods", "ethiopia-methods", "nigeria-methods")
  ...
  script_src_tags <- regmatches(
    html,
    gregexpr('<script src="[^"]+"></script>', html, perl = TRUE, ignore.case = TRUE)
  )[[1]]
