...
output_file <- file.path(project_root, "output", "goal1_reverse_engineer_extensions.html")
...
source(file.path(project_root, "R", "national_prevalence_reference.R"), encoding = "UTF-8")
...
generated_payloads <- reference_payloads_from_reports(reports)
national_prevalence_reference <- read_national_prevalence_reference(
  file.path(project_root, "data", "national_prevalence_reference.csv"),
  generated_payloads$indicator_rows
)
stopifnot(nrow(national_prevalence_reference) == 23L)
stopifnot(!anyDuplicated(national_prevalence_reference[c("country", "indicator")]))
stopifnot(all(national_prevalence_reference$nationalPrevalence >= 0 & national_prevalence_reference$nationalPrevalence <= 100))
stopifnot(sum(national_prevalence_reference$country == "drc") == 8L)
stopifnot(sum(national_prevalence_reference$country == "ethiopia") == 7L)
stopifnot(sum(national_prevalence_reference$country == "nigeria") == 8L)
...
stopifnot(grepl("Match bivariate map dimensions to the standard overview-map footprint", dashboard_css, fixed = TRUE))
stopifnot(grepl(".bivariate-map .geo-map {\n  height: auto;\n  aspect-ratio: 720 / 460;", dashboard_css, fixed = TRUE))
stopifnot(grepl("left: calc(100% + 30px);", dashboard_css, fixed = TRUE))
...
stopifnot(grepl("const nationalPrevalenceRows = {{NATIONAL_PREVALENCE_ROWS}};", interaction_script, fixed = TRUE))
stopifnot(grepl('distributionPanel.querySelector(".eyebrow").textContent = "State ranking: high to low";', interaction_script, fixed = TRUE))
stopifnot(grepl('medianLabel.textContent = "Median across states";', interaction_script, fixed = TRUE))
stopifnot(grepl("Unweighted median of the displayed state values", interaction_script, fixed = TRUE))
stopifnot(grepl('" national prevalence: " + fmt(nationalReference.nationalPrevalence) + "%"', interaction_script, fixed = TRUE))
stopifnot(grepl("Vertical line denotes national prevalence.", interaction_script, fixed = TRUE))
stopifnot(grepl("Prevalence (%)", interaction_script, fixed = TRUE))
stopifnot(grepl(".national-reference-panel .bar-scroll", dashboard_css, fixed = TRUE))
stopifnot(grepl("left: var(--national-position);", dashboard_css, fixed = TRUE))
stopifnot(grepl("referenceContrastColors", interaction_script, fixed = TRUE))
stopifnot(reference_fixed_count('data-tab="methods" role="tab"', rendered_html) == 3L)
stopifnot(reference_fixed_count('<b>Methods Overview</b>', rendered_html) == 3L)
stopifnot(reference_fixed_count('<span>02</span><div><h4>How Rankings and Worsening Are Defined</h4>', rendered_html) == 3L)
stopifnot(reference_fixed_count('<span>03</span><div><h4>Prevalence References</h4>', rendered_html) == 3L)
stopifnot(reference_fixed_count('<span>04</span><div><h4>Change and Bivariate Classification</h4>', rendered_html) == 3L)
stopifnot(reference_fixed_count('<h5>Composite prevalence rank</h5>', rendered_html) == 3L)
stopifnot(reference_fixed_count('<h5>Composite change rank</h5>', rendered_html) == 3L)
stopifnot(reference_fixed_count('<h5>Worsening</h5>', rendered_html) == 3L)
...
stopifnot(reference_fixed_count('"nationalPrevalence":', rendered_html) == 23L)
stopifnot(reference_fixed_count('"reportUrl":', rendered_html) == 23L)
stopifnot(grepl("State ranking: high to low", rendered_html, fixed = TRUE))
stopifnot(grepl("Vertical line denotes national prevalence.", rendered_html, fixed = TRUE))
