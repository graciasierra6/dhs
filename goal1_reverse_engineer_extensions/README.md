...
1. Clone or download the complete `goal1_reverse_engineer_extensions/` folder.
2. Open `goal1_reverse_engineer_extensions.Rproj`.
3. Run:
...
5. Open `output/goal1_reverse_engineer_extensions.html`.
...
- `data/national_prevalence_reference.csv`
...
- all 23 country-indicator prevalence maps, including country-specific
  ...
  color distributions;
- one documented national-prevalence reference for every one of the 23
  country-indicator panels, with the displayed year, indicator definition,
  DHS report or official-survey derivation, table identifier, URL, and
  validation note;
...
```text
goal1_reverse_engineer_extensions/
|-- goal1_reverse_engineer_extensions.Rproj
|-- render.R
|-- validate.R
|-- renv.lock
|-- README.md
...
`output/goal1_reverse_engineer_extensions.html`. If the host requires a landing file,
