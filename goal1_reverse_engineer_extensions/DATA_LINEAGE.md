...
`data/national_prevalence_reference.csv` supplies the vertical reference line
for every indicator-prevalence panel. It contains exactly 23 unique
country-indicator rows (8 DRC, 7 Ethiopia, and 8 Nigeria), and records the
displayed survey year, indicator definition, national prevalence, report or
official-survey derivation, table identifier, source URL, and validation note.
The build rejects missing, duplicate, out-of-range, or wrong-year references.
Published DHS national totals are used directly where the report definition
matches the map. Zero-dose is the complement of the published first-dose
coverage. The custom first-birth-under-20 benchmark uses the same conditional
ever-birth denominator as the state estimates and is not replaced with the
different standard report denominator.
...
`output/goal1_reverse_engineer_extensions.html`.
