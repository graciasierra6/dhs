...
profile_image_bank <- function(chart_type, reports, project_root, manifest) {
  bank <- unlist(lapply(names(reports), function(country) {
    ...
    images <- lapply(rows$asset_path, profile_asset_raw, project_root = project_root)
    stats::setNames(
...
profile_image_bundle_plan <- function(
  chart_type,
  reports,
  project_root,
  manifest,
  max_file_bytes = 20L * 1024L * 1024L
) {
  if (!chart_type %in% c("profiles", "change")) {
    stop("Unknown profile chart type: ", chart_type, call. = FALSE)
  }
  bank <- profile_image_bank(chart_type, reports, project_root, manifest)
  prefix <- paste0(
    "(function(root){root.Goal1ProfileImages=root.Goal1ProfileImages||{profiles:{},change:{}};",
    "root.Goal1ProfileImages.", chart_type,
    "=Object.assign(root.Goal1ProfileImages.", chart_type, "||{}, {"
  )
  suffix <- "});})(window);"
  entries <- vapply(names(bank), function(name) {
    encoded <- jsonlite::toJSON(stats::setNames(list(bank[[name]]), name), auto_unbox = TRUE)
    substr(encoded, 2L, nchar(encoded) - 1L)
  }, character(1), USE.NAMES = FALSE)
  fixed_bytes <- nchar(prefix, type = "bytes") + nchar(suffix, type = "bytes") + 1L
  chunks <- list()
  current <- character()
  current_bytes <- fixed_bytes
  for (entry in entries) {
    separator_bytes <- if (length(current)) 1L else 0L
    entry_bytes <- nchar(entry, type = "bytes")
    if (length(current) && current_bytes + separator_bytes + entry_bytes > max_file_bytes) {
      chunks[[length(chunks) + 1L]] <- current
      current <- character()
      current_bytes <- fixed_bytes
      separator_bytes <- 0L
    }
    if (current_bytes + separator_bytes + entry_bytes > max_file_bytes) {
      stop("A single profile image is too large for the configured JavaScript bundle limit.", call. = FALSE)
    }
    current <- c(current, entry)
    current_bytes <- current_bytes + separator_bytes + entry_bytes
  }
  if (length(current)) chunks[[length(chunks) + 1L]] <- current
  plan <- lapply(seq_along(chunks), function(index) {
    content <- paste0(prefix, paste(chunks[[index]], collapse = ","), suffix)
    list(
      chart_type = chart_type,
      chunk_index = index,
      file_name = sprintf("profile_images_%s_%02d.js", chart_type, index),
      content = content,
      bytes = nchar(content, type = "bytes") + 1L
    )
  })
  if (!length(plan) || any(vapply(plan, `[[`, numeric(1), "bytes") > max_file_bytes)) {
    stop("Profile image bundle planning exceeded the configured file-size limit.", call. = FALSE)
  }
  attr(plan, "bank") <- bank
  plan
}

profile_write_image_bundles <- function(reports, project_root, manifest, max_file_bytes = 20L * 1024L * 1024L) {
  plan <- c(
    profile_image_bundle_plan("profiles", reports, project_root, manifest, max_file_bytes),
    profile_image_bundle_plan("change", reports, project_root, manifest, max_file_bytes)
  )
  bundle_directory <- file.path(project_root, "assets", "js")
  expected <- vapply(plan, function(chunk) file.path(bundle_directory, chunk$file_name), character(1))
  existing <- list.files(
    bundle_directory,
    pattern = "^profile_images_(profiles|change)_[0-9]{2}\\.js$",
    full.names = TRUE
  )
  stale <- setdiff(existing, expected)
  if (length(stale)) unlink(stale)
  for (index in seq_along(plan)) {
    writeLines(plan[[index]]$content, expected[[index]], useBytes = TRUE)
    plan[[index]]$path <- normalizePath(expected[[index]], winslash = "/", mustWork = TRUE)
  }
  plan
}

profile_image_script_tags <- function(bundle_plan, src_prefix = "../assets/js") {
  paste(vapply(bundle_plan, function(chunk) {
    paste0('<script src="', src_prefix, "/", chunk$file_name, '"></script>')
